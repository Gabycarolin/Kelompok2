...
class M_data extends CI_Model
{
	...
	{
		$id = $this->session->userdata('id');
		return $rekapTrx = $this->db->query("SELECT * from tb_booking join tb_futsal ON tb_booking.id_futsal=tb_futsal.id_futsal JOIN tb_customer ON tb_booking.id_cust=tb_customer.id_cust where tb_booking.status='Lunas'")->result();
	}

	public function getFilterTgl()
	{
		$tanggal = $this->input->post('waktu_mulai');
		return $filterTgl = $this->db->query("SELECT * from tb_booking join tb_futsal ON tb_booking.id_futsal=tb_futsal.id_futsal JOIN tb_customer ON tb_booking.id_cust=tb_customer.id_cust where tb_booking.status='Lunas' and DATE(waktu_mulai) = DATE('$tanggal')")->result();
	}
}
